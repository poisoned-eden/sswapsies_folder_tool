...
inquirer
  .prompt([
    ...
    //   type: "list",
    //   name: "replaceSrc",
    //   message: "Replace the src directory or copy back to activity folder?",
    //   choices: ["replace", "copy"]
    //   default: false,
    // },
    {
      type: "list",
      name: "previousActivity",
      message: `Which folder do you want to move your work back into?`,
  ...
  .then((answers) => {
      function hasUnsolved(folder) {
        return fs.existsSync(
          path.join(activitiesDir, folder, "Unsolved")
        );
      };
      
      // Directory to copy TO depending on whether it was an activity or a ins demo (hasUnsolved)
      const previousActivity = `${answers.previousActivity}/my_work`;

      // Directory to copy from depending on whether this is an activity or a ins demo (hasUnsolved)
      const srcToCopy = hasUnsolved(answers.srcToCopy)
      ...
      const previousActivityPath = path.join(activitiesDir, previousActivity);

      
      // Copy the work back to previous activity directory
      try {
        info(`Copying ${reactApp} to ${previousActivityPath}/src`);
        fs.copySync(reactAppPath, previousActivityPath);
        success(`Copied your work back to ${previousActivityPath}`);
      } catch (err) {
        error(`Error copying ${reactApp} to ${previousActivityPath}`);
        error(err);
        process.exit(1);
      };
